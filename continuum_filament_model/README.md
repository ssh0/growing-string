...
成長部では、`straight`、`boundary-near`、`buckled-candidate`と成長なしcontrolを、固定meshで`dt, dt/2, dt/4`、空間3段階で比較します。`A1/L`、mode spectrum、onset、peak transverse、curvature RMS、energy、accepted/rejected/eventを保存し、次の量を分離します。
...
`g=0`ではenergy非増加を確認し、`g>0`では成長によるenergy変化を単純な散逸や再mesh jumpと混同しません。比較条件、許容値、分類規則、未解決判定は `notes/p0b_linear_mode_convergence.md` に固定します。P1B.2で`numerically-unresolved`だった境界近傍・座屈候補は、収束不一致や許容値超過があれば同じラベルを維持します。このgateから座屈境界・臨界値・普遍性・相転移を主張しません。
