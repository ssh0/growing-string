...
class ScaleFreeShapeComparisonTests(unittest.TestCase):
    ...
    def test_out_of_range_growth_progress_is_not_aligned(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            observation = self._write_observation(root / "observation", [10.0, 10.4, 10.1, 10.2])
            model = self._write_model(root, [1.0, 1.4, 1.1, 1.2])
